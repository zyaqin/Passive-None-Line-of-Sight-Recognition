...
class MNIST(data.Dataset):
    def __init__(self, root,test_file,filename,train=True, transform=None, target_transform=None, download=False):
        ...
        self.test_data_ori,self.test_data, self.test_labels = torch.load(self.test_file)
    ...
    def download(self):
        ...
        test_set = (read_image_file(self.filename[:-3]+'ys'),
            read_image_file(self.filename),
            read_label_file(self.filename)
        )
...
def read_label_file(path):
    ...
    for i in range(len(img_names)):
        lab.append(int(img_names[i].split('_')[1].split('.')[0]))
...
#img_path=r'C:\Users\Qxslab\Desktop\NLOS\test1'
